...
def _set_font_details_from_card(reviewer, style, selector):
    ...
    card_style = next(
        (
            r
            for r in selector_rules
            if "".join([t.value for t in r.selector]) == selector
        ),
        None,
    )
    if card_style:
...
def typeboxAnsAnswerFilter(self, buf: str) -> str:
    # If no typebox was used, remove pattern
    if not self.typeCorrect:
        return re.sub(self.typeboxAnsPat, "", buf)

    # User’s typed text — keep exactly as typed
    typed = self.typedAnswer

    # Restore newlines for display
    typed = typed.replace("\n", "<br>")

    # Apply card CSS if present
    if self.card.model()["css"] and self.card.model()["css"].strip():
        parser = tinycss.make_parser("page3")
        parsed_style = parser.parse_stylesheet(self.card.model()["css"])
    ...
    font_family = (
        f"font-family: '{self.typeFont}';" if hasattr(self, "typeFont") else ""
    )
    font_size = f"font-size: {self.typeSize}px;" if hasattr(self, "typeSize") else ""
...
<pre class='textbox-output'>{typed}</pre>
</div>
"""

    # Replace the [[typebox:Field]] tag with user output
    return re.sub(self.typeboxAnsPat, html_out.replace("\\", r"\\"), buf)
